...
But what about local housing markets? To get a more granular picture, we could submit a new query to the Census API for finer scale data. I'll explore census tracts in NYC.
...
Now we can do the same mapping procedure as before. But because the overall geographic scope of `data_nyc` is different than the national map of counties, the map space will readjust itself.
...
For the last few maps, I'll switch to a library called `tmap` because it allows more control over how we classify data values into color ramps. Since there are a bunch of census tracts in NYC with no new construction, we need to use a different kind of color ramp to pull out the visual distinction along a smaller range of possible values.

`tmap` uses the same basic 'grammar of graphics' logic that we've seen in `ggplot2`. The primary differences are simply in what various visual layers are called (e.g. `tm_layout` instead of `labs` to add labels to the layout).
...
While visualization is useful, it only *describes* a data pattern. For data storytelling to be persuasive, however, we need to present evidence that tests the significance of the pattern and evaluates uncertainty around the analysis. We'll address this with two related tests: correlation and linear regression.
...
We also need to understand whether the Pearson's r value is statistically 'significant'. To do that, we need to interpret other metrics from these tests including:

-   a t-test, which evaluates whether R is 0 or not 0. Larger values of t indicate that the correlation is more likely to be not 0.

-   p-value for that t-test, which gives a level of statistical confidence (e.g.,"x is correlated with y, at a 95% level of confidence")
...
And we can do the same analysis for our NYC data, stored as a dataframe called `data_NYC`.
...
-   In NYC, the recent movers correlation is much stronger than the new construction pattern.

-   We are confident these variables are correlated with a 99.9% level of significance based on the p-value.

-   We are 95% confident that the actual correlations are somewhere between the first and second values presented in the confidence intervals.
...
We could do the same thing for NYC-specific data by simply changing the `data =` parameter:
